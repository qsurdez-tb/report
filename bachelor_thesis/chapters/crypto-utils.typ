...
#concept[
  ICNML's security rests on a set of cryptographic building blocks for producing unpredictable random values, hashing passwords, and encrypting data. They live in four utility files, and because every security process in the platform reuses the same tools, their strengths and weaknesses propagate everywhere. This chapter examines what each file provides and where each falls short. The code excerpts and the full call-site inventory are in @appendix-crypto.
]
...
First, Python's `random` module is a Mersenne Twister, built for statistical simulation, not security. An observer who collects enough of its output can reconstruct its internal state and predict all future values. The correct tool is the `secrets` module, which draws from the operating system's entropy source.

Second, restricting output to 26 uppercase letters plus 10 digits means each character carries only $log_2(36) approx 5.2$ bits instead of the 8 bits of a full byte. A 20-character value therefore holds about 103 bits of randomness rather than 160 @entropy-wiki, weaker than its length suggests.
...
The wrapper stores its result in a self-describing format: 

```pbkdf2$sha512$<salt>$<iterations>$<digest>```

that records the parameters used. Those recorded parameters let the login flow detect a hash made with outdated settings and upgrade it (@roles-and-permissions). The wrapper defaults to 20 000 iterations. ICNML sets higher values, which @iteration-counts lists by what each hash protects.
...
Two design choices weaken this wrapper. First, integrity is checked by prepending a fixed marker, `"icnml$"`, to every plaintext and confirming it reappears after decryption, discarding the result otherwise. This substitutes for real authenticated encryption. It detects gross corruption but does not cryptographically prove the ciphertext was produced by ICNML, unlike the AES-GCM used in the watermarking work (@watermark-implementation). Second, decryption wraps everything in a bare `except` that returns a plain `"-"` on any failure.
