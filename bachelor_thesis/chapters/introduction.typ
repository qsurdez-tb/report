...
+ *Make the platform maintainable again.* Produce the documentation and the reproducible development environment that a future contributor needs in order to understand, install and work on ICNML, none of which existed before.
...
The work did not stop at description. Where a mechanism was both important and clearly the weakest link, the thesis went further and improved it. The most substantial of these improvements are a full migration of the codebase to a supported version of Python, and a new, robust image watermark that ties every downloaded biometric image to the person who downloaded it.

== Approach and structure of this document

The method throughout was the same, read the existing code closely, explain what it does in terms anyone can follow, and only then improve it where the analysis showed it mattered most. The document follows that same arc, and each chapter opens with a short plain-language summary before going into detail, so that a reader can grasp the essentials without following every technical step.

The chapters fall into four groups.

/ Understanding ICNML : what the platform is and how it is built, its purpose (@icnml-context), the interface its users see (@application-overview), and the structure of its codebase (@repo-struct).

/ The security audit : each protection mechanism examined in turn, the roles and the login process (@roles-and-permissions), the per-donor encryption that makes a donor's data erasable (@per-donor-security), the shared cryptographic utilities underneath it (@crypto-utils), the marking applied to downloaded images (@tattooing), the handling of the biometric data itself (@biometric-data), and the encrypted backups (@backup-security).

/ Modernisation and operations : the migration of the whole system from the end-of-life Python 2.7 to Python 3.11 (@python3-migration), the platform's original automated deployment pipeline (@deployment), and how the modernised system is actually run, both locally and in a production-faithful setup (@running-icnml).

/ A traceable watermark : the most significant piece of new engineering, comprising a survey of how a leaked image can be traced back to its recipient (@state-of-the-art) and the design, implementation and evaluation of the scheme that was built and added to ICNML (@watermark-implementation), followed by the improvements made to the user experience (@ux-improvements).
